<script lang="ts">
  ...
  let currentUserRole: string | null = $state(null);
    ...
    const fetchData = async () => {
      try {
        ...
        if (params.id) {
          ...
          objective_id = parseInt(ids[0], 10); // Extract the first ID
          console.log("Extracted Objective ID:", objective_id);
    ...
    // No need to return a cleanup function unless necessary
    return;
  });
  ...
  const approveAllActionPlans = async () => {
    ...
    try {
      ...
      // If the President approves, check for duplicates before inserting into plan_monitoring
      if (currentUserRole === "president" && plansData) {
        for (const plan of plansData) {
          // Check if the action plan is already in the plan_monitoring table
          const { data: existingEntry, error: checkError } = await supabase
            .from("plan_monitoring")
            .select("id")
            .eq("action_plan_id", plan.id)
            .limit(1); // Limit to one record to minimize load

          if (checkError) {
            console.error(`Error checking for duplicates for plan ID ${plan.id}:`, checkError);
            continue; // Skip this plan if there's an error
          }

          // If no existing entry, insert the plan into plan_monitoring
          if (!existingEntry || existingEntry.length === 0) {
            ...
            if (insertError) {
              console.error(`Error inserting into plan_monitoring for plan ID ${plan.id}:`, insertError);
            }
          }
        }
      }
