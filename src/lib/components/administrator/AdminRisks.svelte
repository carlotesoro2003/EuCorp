<script lang="ts">
  ...
  const fetchVPAndPresidentNames = async () => {
    try {
      ...
      if (error || !data) {
        console.error(
          "Error fetching VP and President names:",
          error || "No data found"
        );
    ...
    try {
      ...
      if (userRole === "president" && data && data.length > 0) {
        ...
        if (approvedRisk.is_approved_president) {
          ...
          if (checkError) {
            console.error(
              "Error checking existing risk_monitoring entry:",
              checkError
            );
            return;
          }
          ...
          if (!existingEntries || existingEntries.length === 0) {
            ...
            if (insertError) {
              console.error(
                "Error inserting into risk_monitoring:",
                insertError
              );
            }
          }
  ...
  const applyFilters = () => {
    displayedRisks = risks.filter((risk) => {
      const matchesSearch =
        `${risk.rrn} ${risk.risk_statement} ${risk.department_name}`
          .toLowerCase()
          .includes(searchQuery.toLowerCase());

      const matchesDepartment =
        departmentFilter === "all" || risk.department_name === departmentFilter;

      const matchesClassification =
        classificationFilter === "all" ||
        risk.classification === classificationFilter;
  ...
  const exportToPDF = () => {
    ...
    if (departmentFilter !== "all")
      filterText.push(`Department: ${departmentFilter}`);
    if (classificationFilter !== "all") {
      const classificationName = classification.find(
    ...
    const totalBudget = filteredItems.reduce(
      (sum, risk) => sum + (risk.budget || 0),
      0
    );
    doc.setFontSize(8);
    doc.text(
      `Exported on: ${exportDate} | Total Records: ${totalExported} | Total Budget: P${totalBudget}`,
      14,
  ...
  const filteredItems = $derived(
    risks
      .filter((risk) => {
        const searchFields =
          `${risk.rrn} ${risk.risk_statement} ${risk.department_name}`.toLowerCase();
        const matchesSearch = searchFields.includes(searchQuery.toLowerCase());
        const matchesDepartment =
          departmentFilter === "all" ||
          risk.department_name === departmentFilter;
        const matchesClassification =
          classificationFilter === "all" ||
          risk.classification === classificationFilter;
        const matchesBudget =
          (!budgetRange.min || risk.budget >= budgetRange.min) &&
          (!budgetRange.max || risk.budget <= budgetRange.max);
      ...
      .sort((a, b) => {
        const aValue = String(a[sortField as keyof Risk]);
        const bValue = String(b[sortField as keyof Risk]);
...
<div class="flex flex-col gap-4 container mx-auto p-6">
  ...
  <button
    ...
    class="md:hidden w-full px-4 py-2 bg-secondary rounded-lg text-left flex justify-between items-center"
  >
    Filters
  ...
  <!-- Filters section -->
  <div
    class={`flex flex-col gap-4 ${showMobileFilters ? "block" : "hidden"} md:flex md:flex-row md:flex-wrap md:justify-between`}
  >
    <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto flex-1">
      ...
      <select
        ...
        class="bg-secondary rounded-lg px-3 py-2 w-full md:w-[200px]"
      >
    ...
    <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto">
      ...
      <button
        onclick={exportToPDF}
        class="flex items-center gap-2 bg-secondary px-4 py-2 rounded-lg hover:bg-secondary/80"
      >
        <Download size={20} />
        Export
      </button>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
      ...
      <div class="flex flex-col sm:flex-row items-center gap-4">
        <select
          ...
          class="bg-secondary rounded-lg px-2 py-1 w-full sm:w-auto"
        >
