<script lang="ts">
    ...
    try {
      ...
      risksMonitoring = data
        .map((item: any) => ({
          ...
          school_year: item.risks.school_year || null,
  ...
  const fetchDropdownOptions = async () => {
    try {
      const [likelihoodData, severityData, controlData, monitoringData] =
        await Promise.all([
  ...
  const handleEdit = (risk: RiskMonitoring) => {
    ...
    assessment = {
      likelihoodRating:
        likelihoodRating.find((r) => r.name === risk.likelihood_rating)?.id ||
        null,
      severity: severity.find((s) => s.name === risk.severity)?.id || null,
      riskControlRating:
        riskControlRating.find((c) => c.name === risk.control_rating)?.id ||
        null,
      riskMonitoringRating:
        riskMonitoringRating.find((m) => m.status === risk.monitoring_rating)
          ?.id || null,
    };
  ...
  const updateRatings = async () => {
    ...
    try {
      ...
      const currentControlRating = riskControlRating.find(
        (r) => r.id === currentControlRatingId
      );
      ...
      const likelihood = likelihoodRating.find(
        (r) => r.id === assessment.likelihoodRating
      )?.symbol;
      const severityValue = severity.find(
        (s) => s.id === assessment.severity
      )?.value;

      if (likelihood && severityValue) {
        const { controlRating } = calculateRiskControlRating(
          likelihood,
          severityValue
        );
        const matchingControlRating = riskControlRating.find(
          (r) => r.symbol === controlRating
        );
        ...
        if (matchingControlRating?.value > currentControlValue) {
          // Grading Increased
          newMonitoringStatusId = riskMonitoringRating.find(
            (r) => r.status === "Grading Increased"
          )?.id;
        } else if (matchingControlRating?.value < currentControlValue) {
          // Grading Decreased
          newMonitoringStatusId = riskMonitoringRating.find(
            (r) => r.status === "Grading Decreased"
          )?.id;
        } else if (matchingControlRating?.value === currentControlValue) {
          // No Change to Grade
          newMonitoringStatusId = riskMonitoringRating.find(
            (r) => r.status === "No Change to Grade"
          )?.id;
        }

        // Update monitoring rating in the UI if it changes
        if (
          newMonitoringStatusId &&
  ...
  const filteredItems = $derived(
    risksMonitoring.filter((risk) => {
      ...
      const matchesLikelihood =
        filterLikelihood === "all" ||
        risk.likelihood_rating?.toLowerCase() ===
          filterLikelihood.toLowerCase();
      const matchesSeverity =
        filterSeverity === "all" ||
        risk.severity?.toLowerCase() === filterSeverity.toLowerCase();
      const matchesControlRating =
        filterControlRating === "all" ||
        risk.control_rating?.toLowerCase() ===
          filterControlRating.toLowerCase();
      const matchesSchoolYear =
        schoolYearFilter === "all" || risk.school_year === schoolYearFilter;

      return (
        matchesSearch &&
        matchesLikelihood &&
        matchesSeverity &&
        matchesControlRating &&
        matchesSchoolYear
      );
    })
  );
...
<div class="flex flex-col gap-4 p-4 container mx-auto">
  ...
  <div class="grid gap-4">
    ...
    <div class="grid sm:grid-cols-4 gap-4">
      ...
      <select
        bind:value={filterControlRating}
        class="bg-secondary rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
      >
        ...
        <thead class="bg-muted/50">
          <tr>
            <th class="px-4 py-3 text-left">
              <button
                onclick={() => toggleSort("rrn")}
                class="flex items-center gap-1"
              >
                RRN
                <ArrowUpDown
                  size={16}
                  class={sortField === "rrn" ? "text-primary" : ""}
                />
              </button>
            </th>
            <th class="px-4 py-3 text-left">
              <button
                onclick={() => toggleSort("risk_statement")}
                class="flex items-center gap-1"
              >
                Risk Statement
                <ArrowUpDown
                  size={16}
                  class={sortField === "risk_statement" ? "text-primary" : ""}
                />
              </button>
            </th>
        ...
        <tbody class="divide-y divide-border">
          {#each paginatedItems as risk (risk.id)}
            <tr class="hover:bg-muted/50">
              ...
              <td class="px-4 py-3"
                ><span
                  class={`px-2 py-1 rounded-full text-xs font-medium ${getSeverityBadgeColor(risk.severity)}`}
                >
                  {risk.severity || "N/A"}
                </span></td
              >
      ...
      <div class="flex flex-col sm:flex-row items-center gap-4">
        ...
        <div class="flex gap-2">
          <button
            ...
            onclick={() => currentPage--}
            class="px-3 py-1 rounded-lg border border-border hover:bg-muted disabled:opacity-50 transition-colors"
            >Previous</button
          >
          <button
      ...
      >
        <div class="bg-card rounded-lg shadow-lg max-w-md w-full">
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="space-y-2">
              ...
              <select
                ...
                class="w-full px-3 py-2 bg-secondary rounded-lg focus:ring-2 focus:ring-ring"
              >
            ...
            <div class="space-y-2">
              ...
              <div class="mt-1 p-2 bg-muted rounded-lg">
                {#if assessment.riskControlRating}
                  {@const rating = riskControlRating.find(
                    (r) => r.id === assessment.riskControlRating
                  )}
                  <p class="text-sm">{rating?.name || "Not Available"}</p>
                {:else}
                  <p class="text-sm text-muted-foreground">
                    Select Likelihood and Severity first
                  </p>
                {/if}
              </div>
            </div>
          ...
          <div class="p-4 flex justify-end gap-2 border-t border-border">
            ...
            <button
              onclick={handleSave}
              class="px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-50"
              >Save Changes</button
            >
          </div>
        </div>
