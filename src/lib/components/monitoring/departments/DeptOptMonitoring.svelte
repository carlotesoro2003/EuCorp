	...
	const fetchOpportunities = async () => {
		try {
			...
			opportunities = data.map((monitoringItem: any) => ({
				...
				target_output: monitoringItem.opportunities?.target_output || "No Target Output",
			...
			goals = opportunities.map((opportunity) => ({
				...
				kpi: opportunity.kpi,
				goal: opportunity.target_output,
			...
			<div transition:slide={{ duration: 200 }} class="bg-background rounded-lg shadow-xl w-full max-w-lg mx-4">
				...
				<div class="p-4">
					...
					<div class="mb-4">
						...
						<p class="text-muted-foreground">{selectedGoal?.kpi}</p>
					</div>
					<div class="mb-4">
					...
					<div class="flex justify-end gap-3">
						...
						<button onclick={evaluateGoal} class="px-4 py-2 text-sm bg-primary text-primary-foreground hover:bg-primary/90 rounded-md" disabled={!evaluationText || selectedGoal?.isSubmitting}>
							{selectedGoal?.isSubmitting ? "Evaluating..." : "Submit"}
						</button>
					</div>
				</div>
			</div>
