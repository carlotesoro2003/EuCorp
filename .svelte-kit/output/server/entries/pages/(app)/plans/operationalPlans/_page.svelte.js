import { N as spread_props, O as slot, P as sanitize_props, Q as ensure_array_like, E as escape_html, J as attr, R as stringify, C as pop, A as push, K as bind_props, S as copy_payload, T as assign_payload } from "../../../../../chunks/index.js";
...
function ObjectivesTable($$payload, $$props) {
  ...
  $$payload.out += `<!----></button></th><th class="px-4 py-3 text-left hidden xl:table-cell">Evaluation Measures</th><th class="px-4 py-3 text-left w-[120px]">Action Plans</th><th class="px-4 py-3 text-left">Plans Created</th></tr></thead><tbody class="divide-y divide-border"><!--[-->`;
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let objective = each_array[$$index];
    $$payload.out += `<tr class="hover:bg-muted/50"><td class="px-4 py-3">${escape_html(objective.name)}</td><td class="px-4 py-3 hidden md:table-cell">${escape_html(objective.strategic_initiatives)}</td><td class="px-4 py-3">${escape_html(objective.kpi)}</td><td class="px-4 py-3 hidden lg:table-cell">${escape_html(objective.persons_involved)}</td><td class="px-4 py-3">${escape_html(objective.target)}</td><td class="px-4 py-3 hidden xl:table-cell">${escape_html(objective.eval_measures)}</td><td class="px-4 py-3"><button class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-sm text-primary bg-primary/10 hover:bg-primary/20 rounded-md" title="View Action Plans">`;
    Eye($$payload, { size: 16 });
    $$payload.out += `<!---->View</button></td><td class="px-4 py-3 text-center"><span class="inline-flex items-center gap-1 px-2.5 py-1 text-sm font-medium bg-red-100 text-red-700 rounded-lg"${attr("title", `${stringify(objective.notApproved)} plans not approved`)}>`;
    Target($$payload, { size: 16 });
    $$payload.out += `<!----> ${escape_html(objective.actionPlanCount)}</span></td></tr>`;
  }
...
function _page($$payload, $$props) {
  ...
  const fetchUserDepartment = async () => {
    try {
      ...
      const { data: profileData, error } = await supabase.from("profiles").select("department_id").eq("id", session.user.id).single();
      if (error) {
  ...
  const fetchObjectives = async () => {
    ...
    loading = true;
    try {
      const { data, error } = await supabase.from("strategic_objectives").select(`
					id,
					name,
      ...
      if (error) {
        ...
        objectives = (data || []).map((objective) => {
          ...
          const departmentPlanCount = actionPlans.filter((plan) => plan.department_id === userDepartmentId).length;
          return {
            ...objective,
            actionPlanCount: departmentPlanCount
            // Only count plans by user's department
          };
        });
      }
    } catch (error) {
  ...
  function $$render_inner($$payload2) {
    ...
    $$payload2.out += `<!----> <input type="text"${attr("value", searchQuery)} placeholder="Search objectives..." class="pl-10 pr-4 py-2 bg-secondary border-secondary rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-ring"></div></div></div> `;
    if (loading) {
      $$payload2.out += "<!--[-->";
